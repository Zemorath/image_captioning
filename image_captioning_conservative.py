...
EMBEDDING_DIM = 128
LSTM_UNITS = 128
EPOCHS = 3  # Reduced for faster testing (increase to 20+ for better results)
BATCH_SIZE = 16
FEATURE_BATCH_SIZE = 16  # Batch size for feature extraction (reduce if OOM issues)
...
def load_captions(filename, max_pairs=10000):
    """
    Load captions from text file with optional limit.
    
    Args:
        filename (str): Path to captions file
        max_pairs (int): Maximum number of image-caption pairs to load
        
    Returns:
        dict: Dictionary mapping image names to list of captions
    """
    print(f"Loading captions from {filename} (max {max_pairs} pairs)...")
    captions_dict = {}
    pairs_loaded = 0
...
def create_sequences(captions_dict, tokenizer, features_dict, max_length=MAX_LENGTH, batch_size=100):
    """
...
def main():
    ...
    try:
        ...
        # Load and preprocess data (limited to 10,000 pairs for memory safety)
        captions_dict = load_captions(caption_file, max_pairs=10000)
